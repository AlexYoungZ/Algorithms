...
public class BoggleSolver {
    ...
    // this data structure is very similar to "TrieST"
    ...
    public Iterable<String> getAllValidWords(BoggleBoard board) {
        ...
        for (int i = 0; i < board.rows(); i++) {
            for (int j = 0; j < board.cols(); j++) {
                ...
                boolean[] visitedPositions = new boolean[board.rows() * board.cols()];
                if (node != null) {
                    exploreBoard(results, board, i, j, visitedPositions, node);
                }
            }
        }
        List<String> sortedResults = new ArrayList<>(new HashSet<String>(results));
    ...
    private void exploreBoard(List<String> words, BoggleBoard board, int x, int y,
                              boolean[] visitedPositions, DictionaryNode node) {
        String v = node.val;
    ...
    public int scoreOf(String word) {
        ...
        if (word == null) throw new IllegalArgumentException("word is mandatory");
